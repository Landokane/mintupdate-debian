...
class ChangelogRetriever(threading.Thread):
    ...
    def run(self):
        ...
        if (self.source_package.startswith("lib")):
            changelog_sources.append("http://packages.debian.org/changelogs/pool/main/%s/%s/%s_%s/changelog" % (self.source_package[0:4], self.source_package, self.source_package, self.version))        
            changelog_sources.append("http://packages.debian.org/changelogs/pool/contrib/%s/%s/%s_%s/changelog" % (self.source_package[0:4], self.source_package, self.source_package, self.version))
            changelog_sources.append("http://packages.debian.org/changelogs/pool/non-free/%s/%s/%s_%s/changelog" % (self.source_package[0:4], self.source_package, self.source_package, self.version))        
        else:
            changelog_sources.append("http://packages.debian.org/changelogs/pool/main/%s/%s/%s_%s/changelog" % (self.source_package[0], self.source_package, self.source_package, self.version))        
            changelog_sources.append("http://packages.debian.org/changelogs/pool/contrib/%s/%s/%s_%s/changelog" % (self.source_package[0], self.source_package, self.source_package, self.version))
            changelog_sources.append("http://packages.debian.org/changelogs/pool/non-free/%s/%s/%s_%s/changelog" % (self.source_package[0], self.source_package, self.source_package, self.version))        
